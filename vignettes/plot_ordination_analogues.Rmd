...
```{r}
gp1_nmds <- ordinate_table(GP1@otu_table, D = NULL, GP1@tax_table, GP1@sam_data,
                           method = "metaMDS", dist_method = "bray")
plot_mvar(gp1_nmds, col = "Phylum", shape = "human")

# DCA
gp1_dca <- ordinate_table(GP1@otu_table, D = NULL, GP1@tax_table, GP1@sam_data,
                           method = "decorana")
...
gp1_dpcoa <- ordinate_table(GP1@otu_table, patristicDist, GP1@tax_table,
                            GP1@sam_data, method = "dpcoa", 
                            tables_to_include = c("l1", "c1"), 
                            scannf = F)
plot_mvar(gp1_dpcoa, col = "Phylum")

# PCA
gp1_pca <- ordinate_table(GP1@otu_table, D = NULL, GP1@tax_table, GP1@sam_data, 
                          method = "pca", tables_to_include = c("li", "co"), scannf = F, 
                          nf = 10)
plot_mvar(gp1_pca, col = "Phylum", facet_vector = c("SampleType", "Phylum"))

# MDS
gp1_mds <- ordinate_table(GP1@otu_table, D = NULL, GP1@tax_table, GP1@sam_data, 
                          method = "pco", dist_method = "euclidean", 
                          scannf = F)
layers_list <- list(list(), list(text = TRUE))
plot_mvar(gp1_mds, col = "Phylum", shape = "human", 
          label = "SampleType", layers_list = layers_list)
```
