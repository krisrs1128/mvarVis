...
deug_pca <- ordi(deug$tab, rows_annot = data.frame(res = deug$result),
                 method = "pca", scannf = F)
plot_mvar(deug_pca, "point-text", col = "res")
```
...
plot_mvar(banque_acm, "point", col = "age")
```
...
plot_mvar(dunedata_hillsmith, "point-text", col = "Ach.mil")
```
...
plot_mvar(dunedata_mix, "point-text-arrow", col = "Ach.mil")
```
